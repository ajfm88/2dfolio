...
import { GameMode, gameModeName } from './core/game-mode.js';
...
import { loadAllAssets } from './data/asset-manifest.js';
...
import { CaveRoom } from './world/cave-room.js';
...
import { noclip } from './world/collision.js';
...
let overworldDataModule = null;

// Cave system
let curtainEffect = null;
let caveRoom = null;
let pendingCaveIndex = null;
let caveContentsData = [];
let caveEntryX = 0;
let caveEntryY = 0;
...
const sfxDetonatedBombs = new WeakSet();
let sfxLowHealthPlaying = false;
let sfxRefillPlaying = false;

// Ring-tinted link sheets
let linkSheetBlue = null;
let linkSheetRed = null;

// Auto-activation items
let stepladder = null;
let raft = null;
let raftTriggeredScroll = false;
...
// Inventory subscreen
let inventorySlide = null;
let inventoryScreen = null;
let redFont = null;
let processedItems = null;
let processedNESItems = null;
let processedNpcs = null;

// Cave data
let caveTextData = null;
let caveTypesData = [];

// Recorder/Flute data + state
let fluteSecretRoomIds = [];
let moduleLevelSecretsData = null;
let teleportingLevelIndex = 0;
let recorderEffect = null;

// Enemy system (G1/G2)
let spawnManager = null;
let dropEngine = null;
let enemySpawnData = null;
let itemsDataForDrops = null;
let enemySheet = null;
...
let dungeonSpikeTraps = [];
let dungeonStatues = null;
let dungeonPushBlock = null;
let dungeonRoomItem = null;
let dungeonRoomItemActive = false;
let dungeonStairsPos = null;
...
function applyTransparencyKey(image) {
...
async function init() {
  try {
    ...
    overworldDataModule = overworldJson;
    const itemsData = itemsJson;
    const secretsData = secretsJson;
    caveTextData = caveTextJson;
    enemySpawnData = enemySpawnJson;
...
window.__zelda = {
  ...
  step(frames = 1) {
    ...
    return { frame: frameCount, mode: gameModeName(gameMode), level: currentLevel };
  },
  // Live handles for inspection.
  get roomFlagBlocks() { return dungeonRoomFlags; },
  get gameMode() { return gameModeName(gameMode); },
...
function startGameFromSlot(index) {
...
function placeAtDungeonEntrance(level) {
...
function applyBubbleJinx(objectType) {
...
function initDungeonRoomObjects() {
  ...
  // Pre-existing stairs: if this room connects to a cellar and the stairs
  // are already in the tile layout (not behind a push-block trigger 5), detect them.
  const cellar = dungeonManager.getCellarForRoom(room.id);
  if (cellar) {
    if (room.secretTrigger !== 5) {
      dungeonStairsPos = dungeonManager.findStairsPosition() ?? { x: 0xD0, y: 0x60 };
    } else if (dungeonManager.roomFlags.isSecretFound(room.id)) {
...
function lookupCaveText(caveIndex) {
...
function renderDebugOverlay() {
  ...
  const modeStr = gameModeName(gameMode);
...
function updateGameplay() {
  ...
  if (overworld.isTransitioning) {
    ...
    if (!overworld.isTransitioning && link.inventory.raft) {
      ...
      if (raft && raftTriggeredScroll) {
        raft.suppressArrival();
      }
      raftTriggeredScroll = false;
    }
    return;
  }
  ...
  if (!stepladder && link.inventory.ladder && !link.halted) {
    ...
    if (LADDER_ROOMS_OW.includes(screenId)) {
...
function updateDungeonGameplay() {
  ...
  const collision = dungeonManager.collision;
  ...
  if (spawnManager) {
    ...
    const capturer = spawnManager.activeEnemies.find(
      (e) => e instanceof LikeLike && e.capturing,
    );
    if (capturer) {
      link.halted = true;
    ...
    const grabber = spawnManager.activeEnemies.find(
      (e) => e instanceof Wallmaster && e.grabbed,
    );
    if (grabber) {
...
function handleItemPickup(itemId) {
  if (!link || !overworld) return;
...
function handleDungeonItemPickup(itemId) {
  if (!link) return;
  playPickupSfx(itemId);
  const masked = itemId & 0x3f;
  const inv = link.inventory;
...
function handleDungeonItemGeneric(masked) {
  if (!link) return;
  const inv = link.inventory;
  switch (masked) {
    case 0x01: inv.sword = Math.max(inv.sword, 1); break;
...
function readCaveInputDirection() {
  if (input.isHeld(Action.Up)) return Direction.Up;
  if (input.isHeld(Action.Down)) return Direction.Down;
  if (input.isHeld(Action.Left)) return Direction.Left;
  if (input.isHeld(Action.Right)) return Direction.Right;
...
function handleRespawn(countDeath = true) {
...
function renderDungeonEntities() {
  if (!link) return;
  const ctx = renderer.ctx;
  ...
  // Mask baked-in stairs from dungeons-map.png when they shouldn't be visible.
  if (!dungeonStairsPos && dungeonManager && !dungeonManager.inCellar) {
    const bakedStairs = dungeonManager.findStairsPosition();
    if (bakedStairs) {
      dungeonManager.maskBakedRoomItem(renderer, bakedStairs.x, bakedStairs.y);
    }
  }
