...
const FRAME_CYCLE = [0, 1, 2, 1, 0, 1, 2, 1, 3];
const ATTR_CYCLE = [0, 0, 0, 0x40, 0x40, 0xC0, 0x80, 0x80, 1];

// Z_07.asm:3831 diagonal speed tables for homing return
const QSPEED_FRACS_X = [0x80, 0x78, 0x70, 0x68, 0x60, 0x4C, 0x36, 0x20, 0x00];
const QSPEED_FRACS_Y = [0x00, 0x20, 0x36, 0x4C, 0x60, 0x68, 0x70, 0x78, 0x80];
...
export class Boomerang {
  ...
  // Animation
   _cycleIndex = 0;
   _animCounter = BOOMERANG_ANIM_COUNTER_RESET;

  constructor(x, y, dirX, dirY, isMagic) {
    this._x = x;
    this._y = y;
    this._dirX = dirX;
    this._dirY = dirY;
  ...
  get isActive() { return this._state !== BoomerangState.Dead; }
  get isCaught() { return this._isCaught; }
  get isMagic() { return this._isMagic; }
  ...
  // Z_07.asm:3831 — diagonal homing using speed fraction tables
   moveToward(targetX, targetY, halfSpeed) {
...
function calcDiagonalSpeedIndex(dx, dy) {
