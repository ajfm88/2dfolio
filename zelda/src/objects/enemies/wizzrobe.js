...
export class BlueWizzrobe extends Enemy {
   _walkTimer = BLUE_WALK_MIN + (Math.floor(Math.random() * 0x80));
...
export class RedWizzrobe extends Enemy {
   _stateByte = 0xff; // counts down each frame, wrapping $00 -> $FF
   _animCounter = 0;
   _didRelocate = false;
