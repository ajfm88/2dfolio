...
export class ManhandlaHand extends Enemy {
  constructor(
    x, y, hp, spawnCloudFrames, _cardinal,
  ) {
...
export class ManhandlaCenter extends Enemy {
   _hands = [];
   _dirMask = DIR8[Math.floor(Math.random() * DIR8.length)];
   _retarget = RETARGET_FRAMES;
   _speedFrac = START_SPEED_FRAC;
   _speedWhole = 0;
   _speedAccum = 0;
   _frame = 0;

  constructor(x, y, hp, spawnCloudFrames) {
    super(x, y, MANHANDLA, hp, spawnCloudFrames);
