...
export function getEnemyHp(objectType, hpPairs) {
...
export class Enemy {
  ...
  get isDead() { return this._state === EnemyState.Dead; }
  get isActive() {
    return this._state !== EnemyState.Dead && this._state !== EnemyState.Dying;
  }
  get isSpawning() { return this._state === EnemyState.Spawning; }
  get isDying() { return this._state === EnemyState.Dying; }
  get isStunned() { return this._state === EnemyState.Stunned; }
  ...
  isImmuneToDamageType(damageBit) {
  ...
  takeDamage(damage, fromDirection, _hitContext) {
  ...
  blocksAttackFrom(_weaponDirection) {
    return false;
  }
  ...
  // Helpers for subclasses
   ...
   moveOnePixel(collision, screen, checkTiles = true) {
   ...
   tickWalkAnimation(counterReset = 6) {
    this._walkAnimCounter++;
  ...
  // NES direction toward Link
   directionTowardLink(linkX, linkY) {
...
function getEnemyColor(objectType) {
  switch (objectType) {
