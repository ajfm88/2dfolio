...
import { Enemy, getEnemyHp } from './enemy.js';
...
export class SpawnManager {
  ...
  spawnForScreen(screen, entryDirection, collision) {
    this._enemies = [];
    this._projectiles = [];
    this._frozen = false;
    this._frozenTimer = 0;
  ...
  // Z_04.asm:1757 CheckZora — one zora on a water tile if none is already active.
   ...
   resolveEnemyTypes(monsterListId) {
  ...
  // room's foe-count nibble (else a boss room would spawn 3 Aquamentus).
   clampBossCount(monsterListId, count) {
  ...
  update(
    ...
    const ctx = { collision, screen, linkX, linkY, bombs, fluteActive };
...
function directionToSpawnList(entryDirection) {
