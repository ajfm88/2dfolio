...
const GREEN_FIELD = (r, g, b) =>
  r < 3 && Math.abs(g - 128) < 3 && b < 3;
const GREY_BOX = keyColor(...SPRITE_BOX_GREY);

export function initDungeonEnemySprites(image) {
...
let overworldEnemySource = null;

export function initOverworldEnemySprites(image) {
...
export function initDeathSprites(image) {
...
export function drawDeathFrame(renderer, frameIndex, dx, dy) {
...
export function initSpawnSprites(image) {
  const canvas = document.createElement('canvas');
  canvas.width = image.width;
  canvas.height = image.height;
  const ctx = canvas.getContext('2d');
...
export function drawSpawnFrame(renderer, frameIndex, dx, dy) {
