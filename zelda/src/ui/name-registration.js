...
import { SAVE_SLOT_COUNT, MAX_NAME_LENGTH } from '../save/save-manager.js';
...
export class NameRegistrationScreen {
    board = new NameBoard();
   names = [];        // live 8-char buffers, one per slot
   editable = [];    // false for slots already registered
   slotCursor = 0;              // 0..2 or END_TARGET
   nameCursor = 0;              // 0..MAX_NAME_LENGTH-1
   heldDir = null;
   repeatTimer = 0;
   cursorVisible = true;
   cursorTimer = 0;
  ...
  reset(slots) {
   ...
   applyDir(dir, touchVertical = false) {
