 ...
 */
export const WORLD_FLAG_BLOCKS = ["overworld", "uw1q1", "uw2q1"];
...
/** Cheat code names — grant all items/upgrades on game start. */
const CHEAT_NAMES = ["AJFM88", "SIRHALLI"];

/**
 * True when the registered name's first 5 characters are ZELDA.
 * Extra characters are ignored, matching the NES 5-byte compare.
 */
export function nameStartsSecondQuest(name) {
  return (
    name
      .toUpperCase()
      .padEnd(ZELDA_NAME.length, " ")
      .slice(0, ZELDA_NAME.length) === ZELDA_NAME
  );
}

/** True when the name exactly matches one of the cheat codes (case-insensitive). */
export function nameIsCheatCode(name) {
  return CHEAT_NAMES.includes(name.toUpperCase().trim());
}

function emptySavedInventory() {
...
function emptyWorldFlags() {
  const worldFlags = {};
  for (const block of WORLD_FLAG_BLOCKS) {
    worldFlags[block] = new Array(ROOM_FLAG_BLOCK_SIZE).fill(0);
  }
  return worldFlags;
}

/** NES SwitchProfileToSecondQuest: empty items, 3 hearts, 8 max bombs, flags cleared. */
export function freshSecondQuestState() {
  return {
...
function coerceFlagBlock(raw) {
  const out = new Array(ROOM_FLAG_BLOCK_SIZE).fill(0);
...
function coerceState(raw) {
  ...
  const rawStats =
    typeof r.stats === "object" && r.stats !== null ? r.stats : {};
  const rawInv =
    typeof r.inventory === "object" && r.inventory !== null ? r.inventory : {};
  const rawFlags =
    typeof r.worldFlags === "object" && r.worldFlags !== null
      ? r.worldFlags
      : {};
  ...
  return {
    ...
    visitedScreens: Array.isArray(r.visitedScreens)
      ? r.visitedScreens.filter((v) => typeof v === "number")
      : [],
  };
}
