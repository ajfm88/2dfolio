...
export class DungeonStatues {
  ...
  constructor(uniqueRoomId) {
    this._pattern = STATUE_ROOM_LAYOUTS.indexOf(uniqueRoomId);
    if (this._pattern < 0) {
      this._base = 0;
      this._count = 0;
      this._timers = [];
  ...
  update(linkX, linkY) {
   ...
   makeFireball(posIndex, linkX, linkY) {
...
function cardinalToward(sx, sy, linkX, linkY) {
  const dx = linkX - sx;
  const dy = linkY - sy;
