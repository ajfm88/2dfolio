...
const OPTION_LABELS = ['CONTINUE', 'SAVE', 'RETRY'];
const OPTION_COUNT = OPTION_LABELS.length;

export class GameOverScreen {
   _selectedOption = GameOverOption.Continue;
   _confirmed = false;
   _flashTimer = 0;
   _isDone = false;
  ...
  update(input) {
    ...
    if (input.isJustPressed(Action.Select) || input.isTouchJustPressed(Action.Down)) {
      this._selectedOption = ((this._selectedOption + 1) % OPTION_COUNT);
      // TODO(K1): play selection changed sound (Tune0Request = $01)
    } else if (input.isTouchJustPressed(Action.Up)) {
      this._selectedOption = ((this._selectedOption + OPTION_COUNT - 1) % OPTION_COUNT);
    }
